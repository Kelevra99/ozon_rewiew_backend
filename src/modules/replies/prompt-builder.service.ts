...
export class PromptBuilderService {
  build(args: {
    ...
    const toneNotes = args.product?.toneNotes || args.user.toneNotes || '';
    const productRules = args.product?.productRules || '';
    const productContextLines = args.product
      ? [
...
Антонация:
preset=${tonePreset}
notes=${toneNotes || 'без дополнительных уточнений'}

Специальные правила по товару:
${productRules || 'нет специальных правил'}
